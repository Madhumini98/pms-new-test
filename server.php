...
if (file_exists($log_file)) {
  ...
  if ($logs !== false) {
    ...
    $log_contents = "Error reading the log file.";
  }
} else {
  $log_contents = "Log file not found or accessible";
}

// Function to filter log lines based on both date range and log type
function filterLogByDateAndType($log_lines, $start_date, $end_date, $log_type) {
  $filtered_lines = array();
  foreach ($log_lines as $line) {
    preg_match('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}).*? - (DEBUG|CRITICAL|ERROR) - (.*?)(?=\. User)/',
$line, $matches);
    $log_date = new DateTime($matches[1]);
    $log_type_found = trim($matches[3]);
    
    if (($log_date >= $start_date && $log_date <= $end_date) && ($log_type === '' || stripos($log_type_found, $log_type) !== false)) {
      $filtered_lines[] = $line;
    }
  }
...
// Read log lines from the $log_contents
$log_lines = explode("\n", $log_contents);

// Check if there's a custom date range filter and log type filter
if (isset($_GET['start_date']) && isset($_GET['end_date']) && isset($_GET['log_type'])) {
  $start_date = new DateTime($_GET['start_date']);
  $end_date = new DateTime($_GET['end_date']);
  $log_type = $_GET['log_type'];

  // Filter log lines based on the date range and log type
  $log_lines = filterLogByDateAndType($log_lines, $start_date, $end_date, $log_type);
} else {
  // If no filter is applied, show all log lines
  $start_date = new DateTime('2023-10-01'); // A date far in the past
  $end_date = new DateTime(); // Current date and time
  $log_type = ''; // No log type filter applied
}
?>
...
<!-- 
    <script>
      document.getElementById("filterForm").addEventListener("submit", function(event) {
          ...
          // Perform AJAX request to fetch filtered log content
          fetch(`filter_logs.php?startDate=${startDate}&endDate=${endDate}`)
              .then(response => response.text())
              .then(data => {
                  document.getElementById("filteredLogContent").innerHTML = data;
              })
              .catch(error => {
                  console.error("Error fetching filtered log content:", error);
              });
      });
    </script>
 -->
        ...
        <div class="main-sidebar">
          <div class="flex h-full w-full flex-col items-center border-r border-slate-150 bg-white dark:border-navy-700 dark:bg-navy-800">
            ...
            <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
              ...
              <a href="server.php" class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" x-tooltip.placement.right="'Server Status'">
...
<!--                          Company Name -->
      ...
      <!-- Main Content Wrapper -->
        <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
          ...
          <div class="log-container mt-5">
            <form class="filter-form mt-5" action="" method="get">
                <label for="start_date">Start Date:</label>
                <input type="date" id="start_date" name="start_date" value="<?php echo $start_date->format('Y-m-d'); ?>" required>

                <label for="end_date">End Date:</label>
                <input type="date" id="end_date" name="end_date" value="<?php echo $end_date->format('Y-m-d'); ?>" required>
                
                <label for="log_type">Log Type:</label>
                <select id="log_type" name="log_type" required class="custom-select-log-type">
                    <option class="custom-option-log-type" value=" ">Select Here</option>
                    <option class="custom-option-log-type" value="Number Plate Not Detected">Number Plate Not Detected</option>
                    <option class="custom-option-log-type" value="Not Register Vehicle">Not Register Vehicle</option>
                    <option class="custom-option-log-type" value="Not Entered Vehicle">Not Entered Vehicle</option>
                    <option class="custom-option-log-type" value="Image Cropping Failed">Image Cropping Failed</option>
                    <option class="custom-option-log-type" value="Process Pass">Process Pass</option>
                    <option class="custom-option-log-type" value="Exit Process Fail">Exit Process Fail</option>
                    <option class="custom-option-log-type" value="Server Error">Server Error</option>
                    <option class="custom-option-log-type" value="OCR Fail">OCR Fail</option>
                    <option class="custom-option-log-type" value="Duplicate Detection">Duplicate Detection</option>
                    <option class="custom-option-log-type" value="Already Entered Vehicle">Already Entered Vehicle</option>
                </select> 
            ...
            if (empty($log_lines)) {
                ?>
                <p>No log entries found for the selected date range and log type.</p>
                <?php
            } else {
                // Reverse the order of log_lines to show the latest entry first
                $log_lines = array_reverse($log_lines);

                // Show only the latest 20 log entries
                $latest_20_log_entries = array_slice($log_lines, 0, 500);

                foreach ($latest_20_log_entries as $line) {
                    ?>
    ...
    <script>
      function resetFilter() {
          // Set the start_date and end_date input values to empty strings to clear the filter
          document.getElementById("start_date").value = "2023-01-01";
          document.getElementById("end_date").value = "";

          // Reset log type selection to default (All)
          document.getElementById("log_type").value = "";

          // Submit the form to display all log entries
...
if (isset($_SESSION['edit'])){
    if ($_SESSION['edit'] == 'success') {
        echo '<script>swal("Success", "Parking spot edit success.", "success");</script>';

    }
    if ($_SESSION['edit'] == 'error') {
        echo '<script>swal("Error", "Parking spot edit Error.", "error");</script>';
...
if (isset($_SESSION['add'])){
    ...
    if ($_SESSION['add'] == 'db_error') {
        echo '<script>swal("Error", "Sever Error! Try again Later.", "error");</script>';

    }
    if ($_SESSION['add'] == 'id_error') {
        echo '<script>swal("Error", "Spot ID Already exit.", "error");</script>';
