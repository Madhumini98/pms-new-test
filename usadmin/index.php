...
<body x-data="" class="is-header-blur" x-bind="$store.global.documentBody">
  ...
  <div id="root" class="min-h-100vh flex grow bg-slate-50 dark:bg-navy-900" x-cloak="">
    ...
    <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
      ...
      <div class="items-center justify-between py-5 lg:py-6">
        <h1 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Clients</h1>
      </div>
      ...
      <div class="items-center justify-between py-5 lg:py-6">
        <h1 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Clients Accounts</h1>
      </div>
      <div class="mt-4 grid grid-cols-1 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
        <div class="card pb-4">
          <div>
            <div id="client_account"></div>
            <?php
  ...
  <script>
    new gridjs.Grid({
      columns: ["Clent", "Mail", "TEL", "Payment", "Info",
        {
          name: 'Action',
          formatter: (_, row) => gridjs.html(`<a href="editform.php?id=${row.cells[0].data}"> <button type="button" class="btn btn-secondary">
                    <i class="fas fa-edit"></i>
                    </button></a> 
                    <a href="addapi.php?id=${row.cells[0].data}"> <button type="button" class="btn btn-secondary">
                    <i class="fas fa-add"></i>
                    </button></a>
                    <a href="data.php?id=${row.cells[0].data}"> <button type="button" class="btn btn-secondary">
                    <i class="fas fa-bar-chart"></i>
                    </button></a>
                    <a href="confirm_delete.php?id=${row.cells[0].data}"> <button type="button" class="btn btn-secondary">
                    <i class="fas fa-trash-alt"></i>
                    </button></a>
                    <button class="mt-3 px-4 rounded-md py-1" "btn h-9 w-full space-x-2" style="background-color: red; color: white;" class="hover:bg-red-focus  focus:bg-red-focus active:bg-red-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90" type="button" class="btn btn-secondary" onclick="confirmDelete()">
          Clear
        </button>`)
        }
      ],
  ...
  <script>
    new gridjs.Grid({
      columns: ["Client", "Mail", "Password", "Role", "User Name",
        {
          name: 'Reset Password',
          formatter: (_, row) => gridjs.html(`<a href="resetform.php?client=${row.cells[0].data}&role=${row.cells[3].data}"> <button type="button" class="btn btn-secondary">
                    <i class="fas fa-edit"></i>
                    </button></a>`)
        }
      ],
    ...
    }).render(document.getElementById("client_account"));
...
if (isset($_SESSION['delete'])) {
  ...
  if ($_SESSION['delete'] == 'delete_success') {
    echo '<script>swal("Success", "Client delete success.", "success");</script>';
  }
  if ($_SESSION['delete'] == 'delete_error') {
    echo '<script>swal("Fail", "Client delete fail.", "error");</script>';
  }
  if ($_SESSION['delete'] == 'Folder delete failed') {
    echo '<script>swal("Fail", "Client delete fail. Folder delete failed.", "error");</script>';
  }
