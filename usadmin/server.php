...
if ($handle) {
    // read the file line by line and store the last 20 lines in an array
    while (!feof($handle)) {
        $log_lines[] = fgets($handle);
        if (count($log_lines) > 20) {
            array_shift($log_lines);
        }
    }

    fclose($handle);
    $log_contents = implode("<br>", $log_lines);
} else {
    $log_contents = "Server is not running or log file is inaccessible.";
}
