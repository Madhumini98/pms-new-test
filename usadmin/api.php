...
$sql = "SELECT `api`, `client`, `info` FROM `api`";
$result = mysqli_query($link, $sql);
$row = mysqli_fetch_all($result);
$json = json_encode($row);

// select 'did' from 'api'
$sql1 = "SELECT `client`,`did` FROM `api`;";
  ...
  <div id="root" class="min-h-100vh flex grow bg-slate-50 dark:bg-navy-900" x-cloak="">
      ...
      <div class="main-sidebar">
        <div class="flex h-full w-full flex-col items-center border-r border-slate-150 bg-white dark:border-navy-700 dark:bg-navy-800">
          ...
          <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
            ...
            <!-- API -->
            <a href="api.php" class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" x-tooltip.placement.right="'API And Device ID'">
    ...
    <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
      ...
      <div class="items-center justify-between py-5 lg:py-6">
        <h1 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Client API Key List</h1>
      </div>
      ...
      <div class="items-center justify-between py-5 lg:py-6">
        <h1 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Device ID List</h1>
      </div>
      <div class="mt-4 grid grid-cols-1 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
        <div class="card pb-4">
          <div>
  ...
  <script>
    new gridjs.Grid({
      columns: [{
          name: 'API Key',
          formatter: (cell) => {
            return cell === '' ? 'Not Assign Yet' : cell === null ? 'Not Assign Yet' : cell.toString();
          },
        }, "Client", {
          name: 'Info',
          formatter: (cell) => {
            if (cell === '' || cell === null) {
              return 'Not Assign Yet';
            } else {
    ...
    new gridjs.Grid({
      columns: [{
        ...
        },
        {
          name: 'Device ID',
          formatter: (cell) => {
            return cell === '' ? 'Not Register Yet' : cell === null ? 'Not Register Yet' : cell.toString();
          },
        },
      ],
      search: true,
...
if (isset($_SESSION['edit'])) {
  if ($_SESSION['edit'] == 'success') {
    echo '<script>swal("Success", "API Info edit success.", "success");</script>';
  }
  if ($_SESSION['edit'] == 'dail') {
    echo '<script>swal("Fail", "API Info Edit fail.", "error");</script>';
  }
  unset($_SESSION['edit']);
}
