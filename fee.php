...
// loop through the array and add a new key "Info"
foreach ($data as &$item) {
  ...
  switch ($key[0]) {
    case 'a':
      $item['Info'] = ($key[2] == 'n') ? 'A Class Normal' : 'A Class Long';
      break;
    case 'b':
      $item['Info'] = ($key[2] == 'n') ? 'B Class Normal' : 'B Class Long';
      break;
    case 'c':
      $item['Info'] = ($key[2] == 'n') ? 'C Class Normal' : 'C Class Long';
      break;
    default:
      $item['Info'] = 'Unknown';
      break;
...
foreach ($data as $item) {
    // Extract the first two values and add them to the output array as a new sub-array along with the "Info" value
    $output[] = array($item[0], $item[1], $item['Info']);
}

$json3 = json_encode($output);
?>
        ...
        <div class="main-sidebar">
          <div class="flex h-full w-full flex-col items-center border-r border-slate-150 bg-white dark:border-navy-700 dark:bg-navy-800">
            ...
            <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
              ...
              <!-- Fees -->
              <a href="fee.php" class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" x-tooltip.placement.right="'Fee Viewer'">
                <svg class="h-7 w-7" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
<!--                          Company Name -->
    ...
    <script>
        new gridjs.Grid({
          columns: [
                    { name: "Fee ID", title: "Fee ID" },
                    { name: "Fee ( LKR. )", title: "Info" },
                    { name: "Info", title: "Info" },
                    {
                      name: "Action",
                      formatter: (_, row) => gridjs.html(`<a href="editfee.php?feeid=${row.cells[0].data}" > <button type="button" class="btn btn-secondary">
