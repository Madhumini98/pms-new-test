...
// Fetch spot details
$sql2 = "SELECT `spot_id`, `parked_vehicle`, `parked_time` FROM `spot` WHERE `parked_vehicle` IS NOT NULL AND `parked_time` IS NOT NULL;";
...
<html lang="en">
  <head>
    ...
    <title>Park'N Pay | Parking map</title>    
    <!-- Include jQuery -->
    ...
    <style>
    .custom-table {
        border-collapse: collapse; /* This property ensures that table borders are collapsed into a single border */
        /*border: 2px solid #FF0000; /* Replace #FF0000 with the desired border color */
    }
    .custom-table th, .custom-table td {
        border: 1px solid #ffffff; /* Apply the same border color to table cells (optional) */
        padding: 8px; /* Add padding for cell content (adjust as needed) */
    }
    .custom-table tr {
      border-left: none;
        border-right: none;
          ...
          <script>
              $(document).ready(function() {
                  setInterval(function() {
                      ...
                      $.ajax({
                          url: 'index-copy.php',  // Use the same file if you want to reload the same page
                          method: 'GET',
                          success: function(response) {
                              // Update the content of the 'contentToReload' div
                              $('#contentToReload').html(response);
                          },
                          error: function() {
        ...
        <div class="main-sidebar">
          <div class="flex h-full w-full flex-col items-center border-r border-slate-150 bg-white dark:border-navy-700 dark:bg-navy-800">
            ...
            <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
              ...
              <!-- Parking Map -->
              <a href="#" class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" x-tooltip.placement.right="'Parking Map'">
             ...
             <!-- <?php
              ...
              <a <?php if($_SESSION['role'] != 'sadmin'){
                ...
                <svg class="h-7 w-7" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <path fill="#64748b" d="M12 1.5C6.21 1.5 1.5 6.21 1.5 12c0 5.79 4.71 10.5 10.5 10.5 5.79 0 10.5-4.71 10.5-10.5 0-5.79-4.71-10.5-10.5-10.5zM12 19.5c-4.14 0-7.5-3.36-7.5-7.5s3.36-7.5 7.5-7.5 7.5 3.36 7.5 7.5-3.36 7.5-7.5 7.5z"></path>
                  <path fill="#64748b" d="M13.5 10.5c0-.828-.672-1.5-1.5-1.5H9v-1.5h1.5c1.38 0 2.5 1.12 2.5 2.5v1.5h-.5zM9 15v-1.5h2c.828 0 1.5-.672 1.5-1.5 0-.828-.672-1.5-1.5-1.5H9V9h1.5c1.38 0 2.5 1.12 2.5 2.5s-1.12 2.5-2.5 2.5H9z"></path>
                </svg>
              </a>
...
<!--                          Company Name -->
       ...
       <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
              <section style="padding-top: 5%;"> 
                  <div style="text-align: center;">   
                      <h1 style= "color: #4F46E5; font-size: 35px; font-weight: bold;">Parking Map</h1>
                  </div>
                  <div class="Show_vehicle" style="display: none; text-align: center; color: #64748b; ">               
                      <div class="container">
                         <!-- <div style="text-align: center;">
                              <h2 style= "font-size: 30px;">Details of the Vehicle</h2>                              
                          </div>-->
                          <div>
                          ...
                          <div>
                              <table>
                                  ...
                                  <tr>
                                      <th>Vehicle Slot</th>
                                      <td id="vehicle-slot"></td>
                                  </tr>
                                  <tr>
                                      <th>Vehicle Category</th>
                                      <td id="vehicle-category"></td>
                                  </tr>
                                  <tr>
                                      <th>Owner</th>
                                      <td id="vehicle-owner"></td>
                                  </tr>
                                  <tr>
                                      <th>Arrive time</th>
                                      <td id="vehicle-arrive-time"></td>
                                  </tr>
                              </table>
                          </div>
                  ...
                  <div class="map" id="map" style="color: #64748b;">
                    <?php
                    
                    // Fetch spot details and organize them into spotTables based on the starting letter of spot_id
                    $sql2 = "SELECT `spot_id`, `parked_vehicle`, `parked_time` FROM `spot` WHERE `parked_vehicle` IS NOT NULL AND `parked_time` IS NOT NULL;";
                    $result2 = mysqli_query($link, $sql2);

                    while ($row = mysqli_fetch_assoc($result2)) {
                        $spot_id = $row['spot_id'];
                        $table_letter = substr($spot_id, 0, 1);
                        $spotTables[$table_letter][] = $row;
                    }
                    ?>

                    <?php
                    // Loop through spotTables and create tables for each letter
                    foreach ($spotTables as $letter => $spots) {
                        echo '<div class="Spot_' . $letter . '">';
                        echo '<h3 style="text-align: center; font-size: 20px; margin: 4px;">' . $letter . ' Parking Spot</h3>';
                        echo '<table class="custom-table">';
                        $columns = 2; // Number of columns per row
                        $spotCount = count($spots);

                        for ($i = 0; $i < $spotCount; $i += 2) {
                            echo '<tr>';
                              // ...
                              for ($j = $i; $j < $i + 2 && $j < $spotCount; $j++) {
                                $spot = $spots[$j];
                                echo '<td';
                                if ($j % 2 == 1 && !empty($spot['parked_vehicle'])) {
                                    // Add a class to the right-side cell when a car is parked
                                    echo ' class="filled-cell"';
                                }
                                echo '>';
                                echo '<h4>' . $spot['spot_id'] . '</h4>';
                                if (!empty($spot['parked_vehicle'])) {
                                    $parkedVehicle = $spot['parked_vehicle'];
                                    $parkedTime = $spot['parked_time'];

                                    // Fetch vehicle details from the 'vehicle' table
                                    $sqlVehicle = "SELECT `vehicle_type`, `vehicle_class`, `owner` FROM `vehicle` WHERE `vehicle_num` = '$parkedVehicle'";
                                    ...
                                    if ($j % 2 == 1) {
                                        // Fill the right-side cell with the car image and add click event
                                        echo '<img class="car-image" src="./img/LCar.png" data-vehicle-num="' . $parkedVehicle . '" data-spot-id="' . $spot['spot_id'] . '" data-parked-time="' . $parkedTime . '" data-vehicle-class="' . $vehicleClass . '" data-owner="' . $owner . '" style="width: 130px; height: auto; display: block; margin: 0 auto; cursor: pointer;">';
                                    } else {
                                        // Fill the left-side cell with the car image and add click event
                                        echo '<img class="car-image" src="./img/RCar.png" data-vehicle-num="' . $parkedVehicle . '" data-spot-id="' . $spot['spot_id'] . '" data-parked-time="' . $parkedTime . '" data-vehicle-class="' . $vehicleClass . '" data-owner="' . $owner . '" style="width: 130px; height: auto; display: block; margin: 0 auto; cursor: pointer;">';
                                    }
                                }
                                echo '</td>';
                              }
    ...
    <script>
        ...
        // Add click event listener to car images
        for (var i = 0; i < carImages.length; i++) {
            carImages[i].addEventListener('click', showVehicleDetails);
        }

        // Function to show vehicle details
        
        function showVehicleDetails(event) {
            var vehicleNum = event.target.getAttribute('data-vehicle-num');
            var spotId = event.target.getAttribute('data-spot-id');
            var vehicleClass = event.target.getAttribute('data-vehicle-class');
            var owner = event.target.getAttribute('data-owner');
            var parkedTime = event.target.getAttribute('data-parked-time');

            // Update vehicle image
            var vehicleImage = document.getElementById('vehicle-image');
            vehicleImage.src = '../images/slt/' + vehicleNum + '.jpg';

            // Update vehicle number
            var vehicleNumber = document.getElementById('vehicle-number');
            vehicleNumber.textContent = vehicleNum;

            // Update vehicle slot
            var vehicleSlot = document.getElementById('vehicle-slot');
            vehicleSlot.textContent = spotId;

            // Update vehicle category
            var vehicleCategory = document.getElementById('vehicle-category');
            vehicleCategory.textContent = vehicleClass;

            // Update vehicle owner
            var vehicleOwner = document.getElementById('vehicle-owner');
            vehicleOwner.textContent = owner;

            // Update vehicle arrive time
            var vehicleArriveTime = document.getElementById('vehicle-arrive-time');
            vehicleArriveTime.textContent = parkedTime;

            // Store selected vehicle details in localStorage
            ...
            var vehicleDetailsSection = document.querySelector('.Show_vehicle');
            vehicleDetailsSection.style.display = 'block';

            // Scroll to vehicle details section
            vehicleDetailsSection.scrollIntoView({ behavior: 'smooth' });
        }


        //Reload the page every 30seconds
        // setTimeout(function() {
        //  location.reload();
        //         }, 30000); 
        ...
        //     // Use jQuery to perform an AJAX request to your PHP script (endpoint)
        //     $.get('index.php', function(data) {
        //         // Update the content of the 'content' div with the response from the server
        //         $('#loadData').html(data.content);
        //     });
        // }

        // // Call the updateContent function every 30 seconds
        ...
        function displayStoredVehicleDetails() {
            // Retrieve stored vehicle details from localStorage
            var storedVehicle = localStorage.getItem('selectedVehicle');

            if (storedVehicle) {
                storedVehicle = JSON.parse(storedVehicle);

                // Update vehicle image
                var vehicleImage = document.getElementById('vehicle-image');
                vehicleImage.src = '../images/slt/' + storedVehicle.vehicleNum + '.jpg';

                // Update other vehicle details
                var vehicleNumber = document.getElementById('vehicle-number');
                vehicleNumber.textContent = storedVehicle.vehicleNum;

                var vehicleSlot = document.getElementById('vehicle-slot');
                vehicleSlot.textContent = storedVehicle.spotId;

                var vehicleCategory = document.getElementById('vehicle-category');
                vehicleCategory.textContent = storedVehicle.vehicleClass;

                var vehicleOwner = document.getElementById('vehicle-owner');
                vehicleOwner.textContent = storedVehicle.owner;

                var vehicleArriveTime = document.getElementById('vehicle-arrive-time');
                vehicleArriveTime.textContent = storedVehicle.parkedTime;
