            ...
            <!-- Right: Header buttons -->
             <div class="-mr-1.5 flex items-center space-x-2">
              ...
              <button @click="$store.global.isDarkModeEnabled = !$store.global.isDarkModeEnabled" class="btn h-8 w-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25">
                <svg x-show="$store.global.isDarkModeEnabled" x-transition:enter="transition-transform duration-200 ease-out absolute origin-top" x-transition:enter-start="scale-75" x-transition:enter-end="scale-100 static" class="h-6 w-6 text-amber-400" fill="#FBBF24" viewbox="0 0 24 24">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" x-show="!$store.global.isDarkModeEnabled" x-transition:enter="transition-transform duration-200 ease-out absolute origin-top" x-transition:enter-start="scale-75" x-transition:enter-end="scale-100 static" class="h-6 w-6 text-amber-400" viewbox="0 0 20 20" fill="#FBBF24">
      ...
      <!-- Main Content Wrapper -->
        <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
              <section style="padding-top: 5%;"> 
                  ...
                  <?php
                    $sqlTotalSpots = "SELECT COUNT(*) as total_spots FROM `spot`";
                    $resultTotalSpots = mysqli_query($link, $sqlTotalSpots);

                    if ($resultTotalSpots) {
                      $row = mysqli_fetch_assoc($resultTotalSpots);
                    ...
                    ?>
                      ...
                      <div class="Spot_1">
                          <h3 style="text-align: center; font-size: 20px; margin: 4px;">1st Parking Spot</h3>
                          <table>
                              <?php
                              for ($i = 1; $i <= ceil($totalSpots/6); $i++) {
                                  $row = mysqli_fetch_assoc($result2);
                                  echo '<tr>';
                                      echo '<td class="remove-left-border">';
                                      echo '<h4>' . $row['spot_id'] . '</h4>';
                                      if (!empty($row['parked_vehicle'])) {
                                          $parkedVehicle = $row['parked_vehicle'];
                                          $sql3 = "SELECT `vehicle_num`, `vehicle_class`, `owner` FROM `vehicle` WHERE `vehicle_num`='$parkedVehicle';";
                                          $result3 = mysqli_query($link, $sql3);
                                          $vehicleData = mysqli_fetch_assoc($result3);

                                          echo '<img class="car-image" src="./img/RCar.png" data-vehicle-num="' . $vehicleData['vehicle_num'] . '" data-spot-id="' . $row['spot_id'] . '" data-vehicle-class="' . $vehicleData['vehicle_class'] . '" data-owner="' . $vehicleData['owner'] . '" data-parked-time="' . $row['parked_time'] . '"style="width: 130px; height: auto; display: block; margin: 0 auto;">';
                                      }
                                      echo '</td>'; 

                                      echo '<td class="remove-right-border">';
                                      $row = mysqli_fetch_assoc($result2);
                                      echo '<h4>' . $row['spot_id'] . '</h4>';
                                      if (!empty($row['parked_vehicle'])) {
                                          $parkedVehicle = $row['parked_vehicle'];
                                          ...
                                          echo '<img class="car-image" src="./img/LCar.png" data-vehicle-num="' . $vehicleData['vehicle_num'] . '" data-spot-id="' . $row['spot_id'] . '" data-vehicle-class="' . $vehicleData['vehicle_class'] . '" data-owner="' . $vehicleData['owner'] . '" data-parked-time="' . $row['parked_time'] . '"style="width: 130px; height: auto; display: block; margin: 0 auto;">';
                                      }
                                      echo '</td>';                   
                                  echo '</tr>';
                              }
                      ...
                      <div class="Spot_2">
                          <h3 style="text-align: center; font-size: 20px; margin: 4px;">2nd Parking Spot</h3>
                          <table>
                              <?php
                              for ($i = 1; $i <= ceil($totalSpots/6); $i++) {
                                  $row = mysqli_fetch_assoc($result2);
                   ...
                   <div class="Spot_3">
                          <h3 style="text-align: center; font-size: 20px; margin: 4px;">3rd Parking Spot</h3>
                          <table>
                              <?php
                              for ($i = 1; $i <= ceil($totalSpots/6); $i++) {
                                  $row = mysqli_fetch_assoc($result2);
