...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $entered_spot = $_POST['entered_spot'];
  ...
  if (mysqli_num_rows($result_check_registration) > 0) {
      if ($entryExit == 'enter') {
        ...
        //Check if vehicle is already entered
        $sql_check_entered = "SELECT * FROM `spot` WHERE `parked_vehicle`='$vehicleNumber'";
        $result_check_entered = mysqli_query($link, $sql_check_entered);
        // echo 'vehicle entered checked';

        if (mysqli_num_rows($result_check_entered) > 0) {
          // error msg
          $_SESSION['manual_add'] = "Already enterd vehicle";
        }else{
            //if spot id field is filled
            if(isset($_POST['entered_spot']) && $_POST['entered_spot'] !== ''){
              // echo 'spot field checked.';
              // echo $entered_spot;

                //check if spot exists
                $sql_check_spotExist = "SELECT * FROM `spot` WHERE `spot_id`='$entered_spot'";
                $result_check_spotExist = mysqli_query($link, $sql_check_spotExist);
                // echo 'spot result retrieved.';
              
                if (mysqli_num_rows($result_check_spotExist) > 0) {

                      //check if spot is available
                      $sql_check_entered_spot = "SELECT `availability` FROM `spot` WHERE `spot_id`='$entered_spot'";
                      $result_check_entered_spot = mysqli_query($link, $sql_check_entered_spot);
                      // echo 'availability retrieved.';


                      if ($result_check_entered_spot) {
                        $row1 = mysqli_fetch_assoc($result_check_entered_spot);
                        // echo 'availability fetched.';

                        // Check if the 'mode' value is 't'
                        if ($row1['availability'] === 't') {
            
                              // echo ' spot is available.';

                              // Update vehicle status
                              $sql_update_vehicle1 = "UPDATE `vehicle` SET `status`='t',`next_fun`='o' WHERE `vehicle_num`='$vehicleNumber'";
                              $result_update_vehicle1 = mysqli_query($link, $sql_update_vehicle1);

                              if ($result_update_vehicle1) {
                                // echo 'vehicle data updated';


                                  // Update spot table
                                  $current_time = date("Y-m-d H:i:s");
                                  $sql_update_spot1 = "UPDATE `spot` SET `availability`='f',`parked_vehicle`='$vehicleNumber',`parked_time`='$current_time' WHERE `spot_id`='$entered_spot'";
                                  $result_update_spot1 = mysqli_query($link, $sql_update_spot1);

                                  if ($result_update_spot1) {
                                      $_SESSION['manual_add'] = "success";
                                      // echo 'spot data updated';
                          ...
                          $_SESSION['manual_add'] = "Spot Not Available";
                        }
                      } else {
                        $_SESSION['manual_add'] = "Error";
                    }
              }else{
                $_SESSION['manual_add'] = "Spot Does Not Exist";
              }
            //if spot id field is not filled
            }else{
          ...
          if (mysqli_num_rows($result_check_entered) > 0) {
            ...
            // $command = "python3 $pythonScript newUpdate_parking_cost_bill $vehicle_num $user";
            $output = array();
            $returnVar = null;
            // exec($command, $output, $returnVar);
            // echo 'dccjnjndcjnjnnjnjn';
            exec("python3 bill.py " . escapeshellarg($vehicle_num) . " " . escapeshellarg($user) . " 2>&1", $output, $return);
            // echo "Output:\n";
            // print_r($output);
            ...
            if ($result_exit_spot) {
             ...
             // error msg
           $_SESSION['manual_add'] = "Not Parked Vehicle";
          }
      }
  } else {
...
<html lang="en">
  <head>
    ...
    <script>
        function toggleEnteredSpot() {
            var entryExitSelect = document.getElementById("entryExit");
            var enteredSpotInput = document.getElementById("spotIdContainer");

            // Show the entered_spot input by default
            enteredSpotInput.style.display = "block";

            // If "Exit" is selected, hide the entered_spot input
            if (entryExitSelect.value === "exit") {
                enteredSpotInput.style.display = "none";
            }
        }
    </script>
      <!-- Meta tags  -->
        ...
        <div class="main-sidebar">
          <div class="flex h-full w-full flex-col items-center border-r border-slate-150 bg-white dark:border-navy-700 dark:bg-navy-800">
            ...
            <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
              ...
              <a <?php if($_SESSION['role'] != 'sadmin'){
                echo 'style="display: none;"';
              } ?>href="manual_entry.php" class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" x-tooltip.placement.right="'Manual Entry'">
              ...
              <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512" Stroke="currentColor" >
                <style>svg{fill:currentColor}</style><path d="M512 80c8.8 0 16 7.2 16 16v32H48V96c0-8.8 7.2-16 16-16H512zm16 144V416c0 8.8-7.2 16-16 16H64c-8.8 0-16-7.2-16-16V224H528zM64 32C28.7 32 0 60.7 0 96V416c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V96c0-35.3-28.7-64-64-64H64zm56 304c-13.3 0-24 10.7-24 24s10.7 24 24 24h48c13.3 0 24-10.7 24-24s-10.7-24-24-24H120zm128 0c-13.3 0-24 10.7-24 24s10.7 24 24 24H360c13.3 0 24-10.7 24-24s-10.7-24-24-24H248z"/>
                </svg>
              </a>
            </div>
...
<!--                          Company Name -->
      ...
      <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <diV style="display: flex; justify-content: center; align-items: center; height: 100vh;">
           <!-- <div  id= "content">
                <?php
                    // Include the content of manual page
                    //include 'manual/index.php';
                ?>
            </div>-->

            <div class="form-container">
              <h2 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Manual Enter and Exit</h2>

              <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">

                  <div class="form-group">
                      <label for="entryExit">Select Enter or Exit</label>
                      <select class="form-control" id="entryExit" name="entryExit" onchange="toggleEnteredSpot()">
                  ...
                  <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="vehicleNumber" name="vehicleNumber" placeholder="ABCxxxx">
                  </div>
                 
                  <div class="form-group" id="spotIdContainer">
                      <label for="entered_spot">Enter Spot ID</label>
                      <input type="text" class="form-control" id="entered_spot" name="entered_spot" placeholder="A01" style="display:block;">
                  </div>
...
if (isset($_SESSION['edit'])){
    if ($_SESSION['edit'] == 'success'){
        echo '<script>swal("Success", "Fee edit success.", "success");</script>';
    }
    if ($_SESSION['edit'] == 'error'){
        echo '<script>swal("Fail", "Fee edit fail.", "error");</script>';
    }
    unset($_SESSION['edit']);
}
?>
...
if (isset($_SESSION['manual_add'])){
    ...
    if ($_SESSION['manual_add'] == 'Already enterd vehicle') {
      echo '<script>swal("Error", "This vehicle is already entered.", "error");</script>';

  }

  if ($_SESSION['manual_add'] == 'Not Parked Vehicle') {
    echo '<script>swal("Error", "This is not a parked vehicle.", "error");</script>';

}

if ($_SESSION['manual_add'] == 'Spot Not Available') {
  echo '<script>swal("Error", "This spot is not available.", "error");</script>';

}

if ($_SESSION['manual_add'] == 'Spot Does Not Exist') {
  echo '<script>swal("Error", "This spot does not exist.", "error");</script>';
