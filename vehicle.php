...
class CustomPDF extends TCPDF {
    ...
    public function Header() {
        // Add watermark image
        $imagePath = 'images/logos/logo-7.png'; // Adjust the path to your watermark image
        $this->Image($imagePath, 60, 100, 100, 0, '', '', '', false, 300, '', false, false, 0);

        // Add image on top left
        $watermarkPath = 'images/logos/logo-new.PNG';
        $this->Image($watermarkPath, 26, 7, 0, 0, '', '', '', false, 300, '', false, false, 0);

        // Add 'report generated'
      if ($this->getPage() == 1) {
        $this->SetTextColor(192, 57, 43);
        $this->SetFont('helvetica', 'BI', 12);
        $this->SetXY($this->getPageWidth() - 50, 2);
        $this->Cell(0, 7, 'REPORT GENERATED' , 0, 1, 'R');
      
  }
           
        // Add date and time on top right
        $this->SetTextColor(50, 60, 60);
        $this->SetFont('helvetica', '', 12);
        $this->SetXY($this->getPageWidth() - 50, 10);
        $this->Cell(0, 7, 'Date :     ' . date('Y-m-d'), 0, 1, 'R');
        $this->Cell(0, 7, 'Time :         ' . date('H:i:s'), 0, 1, 'R');

        // Add a horizontal line
        $this->SetLineWidth(0.3); // Set line width (adjust as needed)
        $this->SetDrawColor(117, 128, 128); // Set line color (black in this case)
        $this->Line(26, 25, $this->getPageWidth() - 26, 25); // Adjust the position and length of the line
    }
    ...
    // Add image on top left
      // $imagepath1 = 'images/logos/SLTMobitel.png';
      // $this->SetX(70);  // Set X coordinate
      // $this->SetY(-30); // Set Y coordinate
      // $this->Image($imagepath1, '', '', 40, 0, '', '', '', false, 300, '', false, false, 0);

      // // Add image on top left
      // $imagepath2 = 'images/logos/TheEmbryo.png';
      // $this->SetX(120);  // Set X coordinate
      // $this->SetY(-30); // Set Y coordinate
      // $this->Image($imagepath2, '', '', 40, 0, '', '', '', false, 300, '', false, false, 0);

      
      $this->SetY(-20);
      $this->SetX(73);
      $this->SetFont('helvetica', 'I', 8);
      $this->Cell(0, 10, "© Copyright © 2023 Park 'N Pay. All rights reserved " , 0, 0);

      $this->SetY(-15);
      $this->SetX(98);
      $this->SetFont('helvetica', 'I', 8);
      $this->Cell(0, 10, 'Page ' . $this->getAliasNumPage() . ' / ' . $this->getAliasNbPages(), 0, 0);
...
function generatePDF($jsonData) {
    ...
    $pdfData = json_decode($jsonData, true);

    // Create an instance of your custom TCPDF class
    $pdf = new CustomPDF();
    // $pdf->SetAutoPageBreak(false);
    // $pdf->setPrintHeader(false); // Disable header
    $pdf->SetMargins(30, 37, 27, 80);
    $pdf->AddPage();
    ...
    if ($pdf->getPage() == 1) {
        ...
        $pdf->SetTextColor(130, 95, 135);
        $pdf->Cell(0, 0, 'Vehicle Historical Data', 0, 1, 'C');
        $pdf->Ln(10);
    }

    // Calculate the center position for the table
    $tableX = ($pdf->getPageWidth() - $pdf->getMargins()['left'] - $pdf->getMargins()['right'] - 25 * 4) / 2;

    // Set font and output column headers
    $pdf->SetFont('times', 'B', 11);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetX($tableX);
    $pdf->Cell(53, 10, 'DATE AND TIME', 1, 0, 'C'); 
    $pdf->Cell(53, 10, 'VEHICLE NUMBER',1, 0, 'C'); 
    $pdf->Cell(53, 10, 'IN/OUT', 1, 0, 'C'); 
    $pdf->Ln();
    ...
    // $pdf->Cell(40, 10, 'VEHICLE NUMBER', 1);
    // $pdf->Cell(40, 10, 'IN/OUT', 1);
    ...
    foreach ($pdfData as $row) {
        ...
        $pdf->Cell(53, 10, $row[3]. '     ' . $row[0], 1, 0, 'C'); 
        $pdf->Cell(53, 10, $row[1], 1, 0, 'C'); 
        $pdf->Cell(53, 10, $row[2], 1, 0, 'C'); 
        $pdf->Ln();
        $pdf->SetX($tableX);
    }
    
    // Save or output the PDF (change filename as needed)
    $pdf->Output('Vehicle_Historical_Data.pdf', 'D');
    exit; // Prevent further processing of the form
}
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if (file_exists($fileName)) {
        ...
        while (($data = fgetcsv($csvFile, 1000, ",")) !== FALSE) {
            // Assuming the date is in the first column, adjust the column index accordingly
            $csv_date = $data[3];
        ...
        // Check if the "Create PDF" button was clicked
...
$folderPath = "bills/{$_SESSION['client']}"; // Bills folder path
$folderPath2 = "bills"; // Bills folder path

$starting_name = $_SESSION['client']; // Starting name of the bills client name
...
if ($handle = opendir($folderPath)) {
  while (false !== ($entry = readdir($handle))) {
    if ($entry != "." && $entry != ".." && strtolower(pathinfo($entry, PATHINFO_EXTENSION)) === 'pdf' && strpos($entry, $starting_name) === 0) {
      $pdfFiles[] = [$entry];
    }
  }
  closedir($handle);
...
$gridData = json_encode($pdfFiles);
$gridData2 = json_encode($pdfFiles2);
      ...
      <!-- Main Content Wrapper -->
        <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
            <div class="items-center justify-between py-5 lg:py-6">
              <h1 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Vehicles Data</h1>
            </div>
    ...
    echo '
        <div class="items-center justify-between py-5 lg:py-6">
            ...
            <div class="filter-container">
                <form class="filter-form mt-5" name="filter-form" action="" method="get">
                    ...
                    <div class="button-group">
                        <button type="submit" class="m-2">Filter</button>
                        <button type="button" id="reset_button" onclick="resetFilter()">Reset</button>
                        <button type="submit" name="create_pdf" id="create-pdf-button">
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
            <div class="card pb-4">
                <div>
                    ...
                    <?php 
                    // Include any additional PHP code here
                    ?>
                </div>
            </div>
        </div>';
...
?>
        ...
        ?>
            <div class="items-center justify-between py-5 lg:py-6">
              <h1 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Vehicles Bills Gallery</h1>
            </div>
        ...
        ?>
            <div class="items-center justify-between py-5 lg:py-6">
              <h1 class="text-3xl text-primary dark:text-accent-light" style="text-align: center;">Vehicles Bills Gallery for Manual Entry</h1>
            </div>
    ...
    <script>
        new gridjs.Grid({
              columns: [
                  ...
                  {
                      name: "Vehicle Class",
                      formatter: (cell) => gridjs.html(cell.toUpperCase() + " Class")
                  },
                  "Owner",
                  "Phone",
                  "NIC",
                  {
                      name: "IMG",
                      formatter: (cell) => gridjs.html(`<img style="width: 50px;" height="50px" src="images/<?php echo $_SESSION['client'] ?>/${cell}">`)
                  },
                  {
                      name: "Action",
                      formatter: (_,row) => gridjs.html(`<a href="vehicle_delete.php?id=${row.cells[0].data}"> <button type="button" class="btn btn-secondary">
    ...
    <script>
      ...
      new gridjs.Grid({
        ...
        pagination: true,
        data: data
      }).render(document.getElementById("vehicle_data"));
    </script>
...
<script>
      new gridjs.Grid({
        ...
        columns: [
          ...
          {
            name: "Download",
            formatter: (_, row) => gridjs.html(`<a href="bills/<?php echo $_SESSION['client']; ?>/${row.cells[0].data}" download target="_blank" rel="noopener noreferrer">Download</a>`)
          }
        ],
        search: true,
        sort: true,
        pagination: true,
        data: JSON.parse('<?php echo $gridData; ?>'), // Parse JSON data here
        style: {
          
        }
      }).render(document.getElementById("bill_wrapper"));
...
<script>
      new gridjs.Grid({
        ...
        columns: [
          ...
          {
            name: "Download",
            formatter: (_, row) => gridjs.html(`<a href="bills/${row.cells[0].data}" download target="_blank" rel="noopener noreferrer">Download</a>`)
          }
        ],
        search: true,
        sort: true,
        pagination: true,
        data: JSON.parse('<?php echo $gridData2; ?>'), // Parse JSON data here
        style: {
          
        }
      }).render(document.getElementById("bill_wrapper2"));
...
?>
  <script>
        function resetFilter() {
          document.getElementById('start_date').value = '2023-09-01';
        document.getElementById('end_date').value = new Date().toISOString().split('T')[0];

        // Trigger the form submission after resetting dates
...
if (isset($_SESSION['msg'])){
    if ($_SESSION['msg'] == 'delete_success'){
        echo '<script>swal("Success", "Delete success.", "success");</script>';
    }
    if ($_SESSION['msg'] == 'delete_error'){
        echo '<script>swal("Fail", "Delete fail.", "error");</script>';
    }
    unset($_SESSION['msg']);
}
if (isset($_SESSION['reg'])){
    if ($_SESSION['reg'] == 'success'){
        echo '<script>swal("Success", "Registration success.", "success");</script>';
    }
    if ($_SESSION['reg'] == 'fail'){
        echo '<script>swal("Fail", "Register fail.", "error");</script>';
    }
    unset($_SESSION['reg']);
}
if (isset($_SESSION['err-del'])){
  if ($_SESSION['err-del'] == 'err'){
      echo '<script>swal("Fail!", "You cannot delete vehicle as Admin.", "error");</script>';
  }
  unset($_SESSION['err-del']);
}


if (isset($_SESSION['bulkadd'])){
  if ($_SESSION['bulkadd'] == 'success'){
      echo '<script>swal("Success", "Bulk Registration success.", "success");</script>';
  }
  if ($_SESSION['bulkadd'] == 'err'){
      echo '<script>swal("Fail", "Bulk Register fail.", "error");</script>';
  }
  unset($_SESSION['bulkadd']);
}
