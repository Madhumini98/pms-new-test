...
// Function to filter log lines based on date range
function filterLogByDate($log_lines, $start_date, $end_date) {
...
// Check if there's a custom date range filter
if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
  $start_date = new DateTime($_GET['start_date']);
  $end_date = new DateTime($_GET['end_date']);

  // Filter log lines based on the date range
  $log_lines = filterLogByDate($log_lines, $start_date, $end_date);
} else {
      ...
      <!-- Main Content Wrapper -->
        <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
          ...
          <div class="log-container mt-5">
            ...
            if (empty($log_lines)) {
                ?>
                <p>No log entries found for the selected date range.</p>
                <?php
            } else {
