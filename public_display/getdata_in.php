...
$sql = "SELECT * FROM `vehicle_logs` WHERE `gate` = 'in' ORDER BY `timestamp` DESC LIMIT 1 ";
...
if ($row = mysqli_fetch_array($result)) {
    ...
    echo '<img src="' . $imagePath . '" alt="Entered Vehicle">';
    echo '<h3><strong>Vehicle Number:</strong> ' . $row['vehicle_number'] . '</h3>';
    echo '<h3><strong>Enter time:</strong> ' . $row['timestamp'] . '</h3>';
}
