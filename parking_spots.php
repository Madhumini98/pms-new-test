<?php
include 'conn.php';
$sql = "SELECT `spot_id`, `availability`, `type`, `class`, `info` FROM `spot`; ";
$result = mysqli_query($link, $sql);
$row1 = mysqli_fetch_all($result);
$json = json_encode($row1);
/*
// get number of 'a' class spots available
//$sql1 = "SELECT COUNT(*) FROM `spot` WHERE `class` = 'a';";
$sql1 = "SELECT COUNT(*) FROM `spot` WHERE `spot_id` LIKE 'A%';";
$result1 = mysqli_query($link, $sql1);
$row2 = mysqli_fetch_all($result1);
$total_a = $row2[0][0];

// get number of 'b' class spots available
//$sql2 = "SELECT COUNT(*) FROM `spot` WHERE `class` = 'b';";
$sql2 = "SELECT COUNT(*) FROM `spot` WHERE `spot_id` LIKE 'E%';";
$result2 = mysqli_query($link, $sql2);
$row2 = mysqli_fetch_all($result2);
$total_b = $row2[0][0];

// get number of 'c' class spots available
//$sql3 = "SELECT COUNT(*) FROM `spot` WHERE `class` = 'c';";
$sql3 = "SELECT COUNT(*) FROM `spot` WHERE `spot_id` LIKE 'F%';";
$result3 = mysqli_query($link, $sql3);
$row3 = mysqli_fetch_all($result3);
$total_c = $row3[0][0];

// get number of 'blocked' spots available
$sql4 = "SELECT COUNT(*) FROM `spot` WHERE `info` = 'block';";
$result4 = mysqli_query($link, $sql4);
$row4 = mysqli_fetch_all($result4);
$total_blocked = $row4[0][0];*/
...
$sql1 = "SELECT spot_id FROM spot";
$result1 = mysqli_query($link, $sql1);

// Create a dictionary to store the total spots for each starting letter
$totals = [];

// Iterate through the rows and update totals
while ($row = $result1->fetch_assoc()) {
  $spot_id = $row['spot_id'];
  $starting_letter = substr($spot_id, 0, 1);

  if (array_key_exists($starting_letter, $totals)) {
    $totals[$starting_letter]++;
  } else {
    $totals[$starting_letter] = 1;
  }
}


// get number of 'available' spots available
$sql5 = "SELECT COUNT(*) FROM `spot` WHERE `availability` = 't';";
$result5 = mysqli_query($link, $sql5);
$row5 = mysqli_fetch_all($result5);
$total_available = $row5[0][0];

// get number of 'unavailable' spots available
$sql6 = "SELECT COUNT(*) FROM `spot` WHERE `availability` = 'f';";
$result6 = mysqli_query($link, $sql6);
$row6 = mysqli_fetch_all($result6);
$total_parked_spots = $row6[0][0];

// get total spots count
$sql7 = "SELECT COUNT(*) FROM `spot`;";
$result7 = mysqli_query($link, $sql7);
$row7 = mysqli_fetch_all($result7);
$total_spots = $row7[0][0];
?>
      ...
      <div class="main-sidebar">
        <div class="flex h-full w-full flex-col items-center border-r border-slate-150 bg-white dark:border-navy-700 dark:bg-navy-800">
          ...
          <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
            ...
            <!-- Parking Spots -->
            <a href="parking_spots.php" class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" x-tooltip.placement.right="'Parking Spots'">
              <svg class="h-7 w-7" xmlns="http://www.w3.org/2000/svg" fill="none" viewbox="0 0 24 24">
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4 lg:gap-6">
            <div class="card flex-row justify-between p-4">
              <div>
                <p class="text-xs+ uppercase">Total spots</p>
                <div class="mt-8 flex items-baseline space-x-1">
                  <p class="text-xs+ uppercase">
                    <!-- <table>
                           <tr>
                              <td>A Pass - </td><td style="text-align:right;"></*?php echo $total_a; ?></td>
                            </tr>
                            <tr>
                              <td>E Pass - </td><td style="text-align:right;"></*?php echo $total_b; ?></td>
                            </tr>
                            <tr>
                              <td>F Pass -</td><td style="text-align:right;"></*?php echo $total_c; ?></td>
                            </tr>
                          </table>-->
                    <?php
                    // Create an HTML table to display the totals
                    echo '<table>';
                    foreach ($totals as $letter => $total) {
                      echo '<tr>';
                      echo '<td>' . $letter . ' Pass - </td>';
              ...
              <div class="mask is-squircle flex h-10 w-10 items-center justify-center bg-warning/10">
                <i class="fa-solid fa-taxi text-xl text-warning"></i>
              </div>
              <div class="absolute bottom-0 right-0 overflow-hidden rounded-lg">
                <i class="fa-solid fa-taxi translate-x-1/4 translate-y-1/4 text-5xl opacity-15"></i>
              </div>
            </div>
            <div class="card flex-row justify-between p-4">
              <div>
                <p class="text-xs+ uppercase">Total Blocked</p>
                <div class="mt-8 flex items-baseline space-x-1">
                  <p class="text-2xl font-semibold text-slate-700 dark:text-navy-100">
                    <?php echo $total_blocked; ?>
                  </p>
                  <p class="text-xs text-success"></p>
                </div>
              </div>
              ...
              <div class="absolute bottom-0 right-0 overflow-hidden rounded-lg">
                <i class="fa-solid fa-ban translate-x-1/4 translate-y-1/4 text-5xl opacity-15"></i>
              </div>
            </div>
            <div class="card flex-row justify-between p-4">
              <div>
                <p class="text-xs+ uppercase">Total Available</p>
                <div class="mt-8 flex items-baseline space-x-1">
                  <p class="text-2xl font-semibold text-slate-700 dark:text-navy-100">
            ...
            <div class="card flex-row justify-between p-4">
              <div>
                <p class="text-xs+ uppercase">Total Parked</p>
                <div class="mt-8 flex items-baseline space-x-1">
                  <p class="text-2xl font-semibold text-slate-700 dark:text-navy-100">
                    <?php echo $total_parked_spots; ?>
                  </p>
                      ...
                      <div>
                            <div class="mt-8 flex items-baseline space-x-1">
                              ...
                              <button class="btn border border-info font-medium text-info hover:bg-info hover:text-white focus:bg-info focus:text-white active:bg-info/90">
                                <a href="parking_map/" target="_blank">Parking Map</a>
                              </button>
      ...
      columns: [
        "Spot ID",
        {
          ...
          formatter: (_, row) => gridjs.html(
            `${row.cells[1].data === 't' ? '<i class="fas fa-car" style="color: green;"></i>' : '<i class="fas fa-car" style="color: red;"></i>'}`
          ),
        },
        {
          name: "Type",
          formatter: (_, row) => gridjs.html(
            `${row.cells[2].data === 'n' ? 'Light Weight' : row.cells[2].data === 'l' ? 'Long Weight' : row.cells[2].data}`
          ),
        },
        {
          name: "Class",
          formatter: (_, row) => gridjs.html(
            `${row.cells[3].data === 'a' ? 'A Class' : row.cells[3].data === 'b' ? 'B Class' : row.cells[3].data === 'c' ? 'C Class' : row.cells[3].data}`
          ),
        },
        {
          name: "Status",
          formatter: (_, row) => gridjs.html(
            `${row.cells[4].data.toUpperCase()}`
          ),
        },
        {
          name: "Action",
          formatter: (_, row) =>
            gridjs.html(
              ...
              `<a href="editspot.php?spotid=${row.cells[0].data}&class=${row.cells[3].data}&type=${row.cells[2].data}" > <button type="button" class="btn btn-secondary">
                        <i class="fas fa-edit"></i>
                        </button></a>

                        <a href="spot-delete.php?id=${row.cells[0].data}" >
...
if (isset($_SESSION['add'])) {
  if ($_SESSION['add'] == 'success') {
    echo '<script>swal("Success", "Parking spot add success.", "success");</script>';
  }
...
if (isset($_SESSION['delete'])) {
  if ($_SESSION['delete'] == 'delete_success') {
    echo '<script>swal("Success", "Parking spot delete success.", "success");</script>';
  }
  if ($_SESSION['delete'] == 'delete_error') {
    echo '<script>swal("Error", "Parking spot delete Error.", "error");</script>';
  }
...
if (isset($_SESSION['remove'])) {
  ...
  if ($_SESSION['remove'] == 'db_error') {
    echo '<script>swal("Error", "Sever Error! Try again Later.", "error");</script>';
  }
  if ($_SESSION['remove'] == 'id_error') {
    echo '<script>swal("Error", "Spot ID does not exist.", "error");</script>';
  }
  unset($_SESSION['remove']);
}

// if (isset($_SESSION['addSpot'])) {
//   if ($_SESSION['addSpot'] == 'success') {
//     echo '<script>swal("Success", "Parking spot add success.", "success");</script>';
//   }
// }
?>