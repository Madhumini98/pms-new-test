...
// Chart 3 data about client account
if ($_SESSION['role'] == "sadmin") {
  ...
  $sql2 = "SELECT `email`,`role`,`uname`,`password`,`client` FROM `users` WHERE `client`='$client';";
  $result2 = mysqli_query($link2, $sql2);
  $row2 = mysqli_fetch_all($result2);
  $json_acc = json_encode($row2);
}
...
<head>
  ...
  <style>
    ...
    /* CSS for table rows hover effect */
    .table-row:hover {
      background-color: #f2f2f2;
      /* Change to your desired hover background color */
      cursor: pointer;
      /* Change to the desired cursor style */
    }

    /* CSS to increase the gap between two rows */
    tr:not(.thead-light) {
      margin-bottom: 10px;
  ...
  <div id="root" class="min-h-100vh flex grow bg-slate-50 dark:bg-navy-900" x-cloak="">
        ...
        <div class="flex h-full w-full flex-col items-center border-r border-slate-150 bg-white dark:border-navy-700 dark:bg-navy-800">
          ...
          <div class="is-scrollbar-hidden flex grow flex-col space-y-4 overflow-y-auto pt-6">
            ...
            <a <?php if ($_SESSION['role'] != 'sadmin') {
              ...
              <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 384 512" Stroke="currentColor">
                <style>
                  svg {
                    fill: #fbbf24
                  }
            ...
            <a <?php if ($_SESSION['role'] != 'sadmin') {
                  echo 'style="display: none;"';
                } ?>href="payment_option.php" class="flex h-11 w-11 items-center justify-center rounded-lg outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25" x-tooltip.placement.right="'Payment Option'">
    ...
    <nav class="header print:hidden">
      ...
      <div class="header-container relative flex w-full bg-white dark:bg-navy-750 print:hidden">
        ...
        <div class="flex w-full items-center justify-between">
          ...
          <!-- Left: Header buttons -->
          <div class="-ml-1.5 left space-x-2">
            <div class="left"> <!-- Added flex-grow to center the content -->
              <div class="dashboard-header left p-4">
                <script>
                  let state = localStorage.getItem('selectedOption');
                  console.log(state);
                  if (state == 'enable') {
                    document.write("<button class='text-green-500' style='font-size:20px'>Payment <i class='fas fa-toggle-on'></i></button>");
                  } else {
                    document.write("<button class='text-red-500' style='font-size:20px'>Payment <i class='fas fa-toggle-off'></i></button>");
                  }
                </script>
              </div>
            </div>
    ...
    <main @click="$store.global.isSidebarExpanded = false" class="main-content w-full px-[var(--margin-x)] pb-8">
      <div class="mt-4 grid grid-cols-1 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
        ...
        <?php
        if ($_SESSION['role'] == "sadmin") {
          echo '<div class="card pb-4">
          <h3 class="mt-1 text-xl font-semibold text-slate-600 dark:text-navy-100" style="
          text-align: center;">
            User Accounts List
          </h3>
          <div>
      ...
      <!-- Table to show in and out vehicls using csv file
        <div>
      ...
      <!-- Table to show in and out vehicles using parking_system.log file -->
      <div>
        <div class="table-container" style="max-height: 400px; overflow-y: auto;">
          <table id="vehicle-table" class="w-full text-left">
            <thead class="thead-light" style="text-align: center; font-family: 'Helvetica Neue'">
              <tr class="border border-transparent border-b-slate-200 dark:border-b-navy-500">
                <th class="whitespace-nowrap px-3 py-3 font-semibold uppercase text-slate-800 dark:text-navy-100 lg:px-5">Date</th>
                <th class="whitespace-nowrap px-3 py-3 font-semibold uppercase text-slate-800 dark:text-navy-100 lg:px-5">Time</th>
                <th class="whitespace-nowrap px-3 py-3 font-semibold uppercase text-slate-800 dark:text-navy-100 lg:px-5">Vehicle Number</th>
                <th class="whitespace-nowrap px-3 py-3 font-semibold uppercase text-slate-800 dark:text-navy-100 lg:px-5">In/Out</th>
              </tr>
            </thead>
            <tbody style="text-align: center; font-family: 'Helvetica Neue'">
              <?php

              // Read the parking_system.log file and populate the table body
              $logFile = './server_log/parking_system.log';
              $lines = file($logFile);
              $currentDate = date('Y-m-d'); // Get the current date in 'YYYY-MM-DD' format

              $hasDataForCurrentDate = false; // Flag to check if there is data for the current date

              foreach ($lines as $line) {
                if (strpos($line, "Process Pass") !== false) {
                  // Extract date, time, and vehicle number
                  preg_match('/(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2}:\d{2},\d{3}) -.*Detected Vehicle : (.*?)\.(.*)/', $line, $matches);
                  if (count($matches) == 5) {
                    $date = $matches[1];
                    $time = $matches[2];
                    $carNum = trim($matches[3]);
                    $spotId = trim($matches[4]);

                    // Determine "In" or "Out" based on the presence of "Spot" information
                    $inOut = strpos($spotId, "Spot") !== false ? "In" : "Out";

                    if (preg_match('/User : (.*?)$/', $line, $userMatch)) {
                      $user = trim($userMatch[1]);

                      if ($user == $client && $date === $currentDate) {
                        $hasDataForCurrentDate = true; // Found data for the current date

                        // Display the table row
                        echo "<tr class='table-row'><td>$date</td><td>$time</td><td>$carNum</td><td>$inOut</td></tr>";
                      }
                    }
                  }
                }
              ...
              // Check if there is data for the current date, and hide the table if there is none
              if (!$hasDataForCurrentDate) {
                echo "<style>#vehicle-table { display: none; }</style>";
              }
              ?>
            </tbody>
          </table>
          ...
          <div class="absolute inset-0 flex h-full w-full flex-col justify-end">
            <div class="space-y-1.5 rounded-lg bg-gradient-to-t from-[#19213299] via-[#19213266] to-transparent px-4 pb-3 pt-12">
              <div class="line-clamp-2">
                <h1 class="text-xl text-info"></h1>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center text-xs text-slate-200">
                  <p class="flex items-center space-x-1">

                    <span class="line-clamp-1">Total Income - Rs. </span> 
                    <span class="line-clamp-1">Total Income - Rs. </span>
